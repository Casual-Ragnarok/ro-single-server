...
if (!empty($_POST['act'])) {
	...
	if (md5($_POST['auth_num']) != $_POST['authnum_p']) {
		...
		$TempString2="<BR><BR><a href='regist.php'>回上一页</a>";
		$RegPD = 1;
	}
	if (!isAlNum($id)) {
		$TempString1="<BR><font color='#ff0000'>请使用数字或英文作用户名</font>";
		$TempString2="<BR><BR><FONT color=#0000FF><A href='regist.php'>返回</a></FONT>";
		$RegPD = 1;
	}
	...
	if ($RegPD != 1) {
		$query="select userid from login where userid='$id'";
		...
		if ($RegPD != 1) {
		$query="insert into login (account_id, userid, user_pass, sex, email,group_id) values ('$accountno','$id','$pass1','$sex','$email','0')";
		$result = mysql_query($query);
			if ($result) {
				$TempString1="<BR><font color='#ff0000'>帐号注册成功，开始您的仙境之旅吧！</font>";
				$TempString2="<BR><BR><A href='index.php'>返回</a>";
			}else{
	...
	$TempString2="<META http-equiv=refresh content='5; url=regist.php'>";
}
echo "
